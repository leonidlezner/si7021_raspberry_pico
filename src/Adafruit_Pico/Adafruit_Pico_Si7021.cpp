/*!
 * @file Adafruit_Pico_Si7021.cpp based on Adafruit_Si7021.cpp
 *
...
/*!
 *  @brief  Instantiates a new Adafruit_Pico_Si7021 class
 *  @param  *theWire
 *          optional wire object
 */
Adafruit_Pico_Si7021::Adafruit_Pico_Si7021() {
  i2c_dev = new Adafruit_Pico_I2CDevice(SI7021_DEFAULT_ADDRESS, i2c_default);
 ...
 */
bool Adafruit_Pico_Si7021::begin() {
  if (!i2c_dev->begin())
    return false;
 ...
 */
float Adafruit_Pico_Si7021::readHumidity() {
  uint8_t buffer[3] = {SI7021_MEASRH_NOHOLD_CMD, 0, 0};

  if (!i2c_dev->write(buffer, 1))
    return NAN;

  sleep_ms(20); // account for conversion time for reading humidity

  uint32_t start = to_ms_since_boot(get_absolute_time()); // start timeout

  while (to_ms_since_boot(get_absolute_time()) - start < _TRANSACTION_TIMEOUT) {
    ...
    sleep_ms(6); // 1/2 typical sample processing time
  }
 ...
 * timeout
 */
float Adafruit_Pico_Si7021::readTemperature() {
  uint8_t buffer[3] = {SI7021_MEASTEMP_NOHOLD_CMD, 0, 0};

  if (!i2c_dev->write(buffer, 1))
    return NAN;

  sleep_ms(20); // account for conversion time for reading temperature
 ...
 */
void Adafruit_Pico_Si7021::reset() {
...
void Adafruit_Pico_Si7021::_readRevision(void) {
 ...
 */
void Adafruit_Pico_Si7021::readSerialNumber() {
  uint8_t buffer[8];
 ...
 *  @return model value
 */
si_sensorType Adafruit_Pico_Si7021::getModel() { return _model; }

/*!
 *  @brief  Enable/Disable sensor heater
 */
void Adafruit_Pico_Si7021::heater(bool h) {
 ...
 */
bool Adafruit_Pico_Si7021::isHeaterEnabled() {
  uint8_t regValue = _readRegister8(SI7021_READRHT_REG_CMD);
 ...
 */
void Adafruit_Pico_Si7021::setHeatLevel(uint8_t level) {
  _writeRegister8(SI7021_WRITEHEATER_REG_CMD, level);
}

/*******************************************************************/

void Adafruit_Pico_Si7021::_writeRegister8(uint8_t reg, uint8_t value) {
...
uint8_t Adafruit_Pico_Si7021::_readRegister8(uint8_t reg) {
